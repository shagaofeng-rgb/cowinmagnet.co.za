...
function section(id, eyebrow, title, introduction, body) {
  return `<section id='product-${id}' class='product-source-section' data-product-panel role='tabpanel' aria-labelledby='product-tab-${id}'><div class='product-source-heading'><p class='eyebrow'>${escapeHtml(eyebrow)}</p><h2>${escapeHtml(title)}</h2>${introduction ? `<p class='product-section-intro'>${escapeHtml(introduction)}</p>` : ""}</div><div class='product-source-body'>${body}</div></section>`;
}
...
function main(product) {
  ...
  const relatedSection = related(product);
  const navItems = [
    ["overview", "Product overview"], ["how-it-works", "How it works"], ["key-features", "Key features"], ["applications", "Typical applications"], ["helps-solve", "What it helps solve"], ["selection", "Installation and selection"], ["specifications", "Technical specifications"], ["options", "Options and customisation"], ["maintenance", "Operation and maintenance"], ["faq", "FAQ"], ...(relatedSection ? [["related-products", "Related products"]] : []), ["product-inquiry", "Request a quote"]
  ];
  const nav = navItems.map(([id, label], index) => `<button id='product-tab-${id}' type='button' role='tab' aria-controls='product-${id}' aria-selected='${index === 0 ? "true" : "false"}' tabindex='${index === 0 ? "0" : "-1"}' data-product-tab>${label}</button>`).join("");
