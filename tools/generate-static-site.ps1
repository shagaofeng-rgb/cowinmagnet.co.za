...
function HeaderHtml($path, $title, $description, $h1, $body, $schema = "") {
  ...
  $active = if($path -match "/products/"){"Products"}elseif($path -match "/industries/"){"Industries"}elseif($path -match "/solutions/"){"Solutions"}elseif($path -match "/markets/"){"African Markets"}elseif($path -match "/technical-support|/downloads"){"Technical Support"}elseif($path -match "/about/"){"About Us"}elseif($path -match "/blog/"){"Blog"}elseif($path -match "/contact/"){"Contact"}else{"Home"}
  $nav = @(
    @("Home","$base/"),@("Products","$base/products/"),@("Industries","$base/industries/"),@("Solutions","$base/solutions/"),@("African Markets","$base/markets/"),@("Technical Support","$base/technical-support/"),@("About Us","$base/about/"),@("Blog","$base/blog/"),@("Contact","$base/contact/")
  )
  $navLinks = ($nav | ForEach-Object { $cls = if($_[0] -eq $active){" class='active'"}else{""}; "<a$cls href='$($_[1])'>$($_[0])</a>" }) -join ""
...
<body>
  <header class="site-header">
    <a class="brand" href="$base/"><span class="brand-mark">C</span><span><strong>Cowinmagnet</strong><small>South Africa & Africa</small></span></a>
    ...
    <section id="mobile-panel" class="mobile-panel" data-mobile-panel hidden>
      ...
      <div class="mobile-links"><a href="$base/industries/">Industries</a><a href="$base/solutions/">Solutions</a><a href="$base/technical-support/">Technical Support</a><a href="$base/about/">About Us</a><a href="$base/contact/">Contact</a><a href="https://wa.me/8615665135205?text=Hello%2C%20I%20am%20interested%20in%20magnetic%20separation%20equipment.">WhatsApp</a><a class="quote-link" href="$base/request-a-quote/">Request a Quote</a></div>
    </section>
  </header>
  <main>
$body
  ...
  <footer class="footer">
    <section class="footer-main simple">
      <div class="footer-brand"><a class="brand" href="$base/"><span class="brand-mark">C</span><span><strong>Cowinmagnet</strong><small>South Africa & Africa</small></span></a><p>Magnetic separation equipment support for African mining and industrial projects.</p></div>
    ...
    <section class="footer-bottom"><span>(c) 2026 Cowinmagnet South Africa & Africa</span><nav><a href="$base/products/">Products</a><a href="$base/markets/">Markets</a><a href="$base/about/">About</a><a href="$base/contact/">Contact</a><a href="$base/privacy-policy/">Privacy</a><a href="https://www.cowinmagnet.com">Global Website</a></nav></section>
  </footer>
...
WritePage "$base/blog/" "Blog | Cowinmagnet South Africa" "Articles about overband magnets, magnetic separator selection, crusher protection and African operating conditions." "Blog" ((PageHero "<a href='$base/'>Home</a> / Blog" "Blog" "Magnetic Separator Articles" "Search and browse articles for selection and maintenance topics.") + "<section class='section'><form class='filter-panel'><label>Search articles<input data-site-search type='search' placeholder='coal, maintenance, overband'></label><label>Category<select><option>All</option><option>Selection</option><option>Maintenance</option><option>Mining</option></select></label></form><div class='grid'>$(($articles | ForEach-Object {"<a class='card' href='$base/blog/$($_.slug)/'><p class='eyebrow'>$($_.date)</p><h3>$($_.title)</h3><p>$($_.summary)</p></a>"}) -join '')</div></section>")
foreach($a in $articles) {
  $body = (PageHero "<a href='$base/'>Home</a> / <a href='$base/blog/'>Blog</a> / $($a.title)" "Article" $a.title $a.summary) + "<section class='section layout'><article class='panel'><p><strong>Date:</strong> $($a.date)</p><h2>Table of contents</h2><ul><li>Overview</li><li>Selection factors</li><li>Related products</li></ul><h2>Article content</h2><p>$($a.summary) Equipment must be selected according to verified operating data, not generic assumptions.</p></article><aside class='panel'><h3>Related products</h3><a href='$base/products/permanent-magnetic-equipment/permanent-overband-magnetic-separator/'>Permanent Overband Magnetic Separator</a><a class='button primary' href='$base/request-a-quote/'>Enquire</a></aside></section>"
  WritePage "$base/blog/$($a.slug)/" "$($a.title) | Cowinmagnet Blog" "$($a.summary)" $a.title $body
}
...
$searchIndex += $articles | ForEach-Object { [pscustomobject]@{title=$_.title; type="Blog"; url="/en-za/blog/$($_.slug)/"; summary=$_.summary} }
